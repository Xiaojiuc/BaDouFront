<template>
    <div class="address-book-dialog">
        ...
        <div slot="footer" v-if="isShowPage" class="col-12 d-flex justify-content-center justify-content-sm-between flex-wrap"
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            selectorConfig() {
                return {selectorData: this.selectorData, searchCondition: this.searchCondition}
            },
        },
        methods: {
            loadTableList() {
                ...
                if (field.dataSourceType === 1 || field.dataSourceType === '1') {
                    /* 列表行列 */
                    this.columns = [{'label': '文本', 'prop': 'text', 'name': 'text'}, {'label': '字典值', 'prop': 'id', 'name': 'id'}]
                    /* 列表数据 */
                    this.chooseListData = this.config.dicData[field.dic]
                }
                /* 自定义数据 */
                else if (field.dataSourceType === 2 || field.dataSourceType === '2') {
                    return
                ...
                /* 模型数据 */
                else if (field.dataSourceType === 3 || field.dataSourceType === '3') {
                    this.defaultSearchParams = Object.assign([], this.config.defaultParams)
                    /* 加载列 */
                    this.loadModule()
                    /* 获取模型数据 */
                    this.loadModuleData()
            ...
            loadModule() {
                ...
                this.post(this.INTERFACE.moduleListConfig, params, (isSuccess, res) => {
                    let arr = []
                    let moduleDisplay = JSON.parse(res.moduleDisplay)
                    moduleDisplay.filter(f => !f.isHidden).forEach(f => {
                        arr.push({
                            prop: f.name,
                            label: f.display,
            ...
            loadModuleData() {
                ...
                this.post(this.INTERFACE.moduleListData, params, (isSuccess, res) => {
                     ...
                     * 删除当前页所有记录的时候会出现这个情况
                     */
                    if (this.chooseListData.length === 0 && this.pagination.pageNo > 1) {
