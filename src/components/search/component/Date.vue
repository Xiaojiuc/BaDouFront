...
<template>
  <div class="search-value">
    ...
    <el-popover
      ...
      width="400"
      trigger="click">
      <el-radio v-model="tmpData.dateType" label="0">
        <el-date-picker
      ...
      <el-radio v-model="tmpData.dateType" label="1">
        在过去的
        <el-input type="text" v-model="tmpData.afterTime"></el-input>
        <el-dropdown>
                <span class="el-dropdown-link">
                  {{tmpData.afterDateMeas}}<i class="el-icon-arrow-down el-icon--right"></i>
                </span>
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item v-for="item in measurements" :key="item.text" @click.native="changeMeasurement('after', item.text)" >{{item.text}}</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
        之内
      </el-radio>
      <el-radio v-model="tmpData.dateType" label="2">
        超过
        <el-input type="text" v-model="tmpData.beforeTime"></el-input>
        <el-dropdown>
                <span class="el-dropdown-link">
                  {{tmpData.beforeDateMeas}}<i class="el-icon-arrow-down el-icon--right"></i>
                </span>
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item v-for="item in measurements" :key="item.text" @click.native="changeMeasurement('before', item.text)">{{item.text}}</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
        之前
      </el-radio>
      <el-button type="primary" @click="search">搜索</el-button>
      <el-button class="text-style" slot="reference" plain>{{showText}}</el-button>
    </el-popover>
  </div>
</template>

<script>
  import { Dropdown,DropdownMenu,DropdownItem,Popover,DatePicker,Radio } from 'element-ui'
  ...
  export default {
    ...
    methods: {
      ...
      isEffectiveData () {
        ...
        if (this.tmpData.dateType === '0' && (this.tmpData.startTime || this.tmpData.endTime)) {
          return true;
        } else if (this.tmpData.dateType === '1' && this.tmpData.afterTime) {
          return true
        } else if (this.tmpData.dateType === '2' && this.tmpData.beforeTime) {
          return true;
        }
        return false
      },
      ...
      getSelectorValue () {
        ...
        if (this.effectiveData.dateType === '0') {
          ...
          result.value = {
            ...
            measurement: this.getMeasurementValue(this.effectiveData.afterDateMeas)
          }
        } else if (this.effectiveData.dateType === '2') {
          result.value = {
            dateType: this.effectiveData.dateType,
            afterTime: this.effectiveData.beforeTime,
            measurement: this.getMeasurementValue(this.effectiveData.beforeDateMeas)
          }
        }
        return result
      },
      ...
      setValue (val) {
        if (val.dateType == 0 && (val.startTime || val.endTime)) {
          ...
          this.effectiveData.afterDateMeas = this.getMeasurementText(val.measurement)
        } else if (val.dateType == 2 && val.beforeTime) {
          this.effectiveData.dateType = '2'
          this.effectiveData.beforeTime = val.beforeTime
          this.effectiveData.beforeDateMeas = this.getMeasurementText(val.measurement)
        }
        this.setShowText()
        this.echoData()
      },
