  ...
  export default {
    ...
    methods: {
      ...
      filterDelete (filterInfo, index) {
        this.$confirm(`确定要删除过滤器 [${filterInfo.name}] 吗？`, '删除', {
          ...
          this.post(url, {ids: filterInfo.id}, (isSuccess, res) => {
            if (res.hasOk) {
      ...
      filterSave(filterInfo, callback) {
        ...
        this.post(url, filterInfo, (isSuccess, res) => {
          if (res.hasOk) {
      ...
      shareFilter() {
        ...
        this.post(url, {filterIds: filterIds.join(','), userIds: userIds.join(',')}, (isSuccess, res) => {
          if (res.hasOk) {
    ...
    watch: {
      mdCode () {
        ...
        this.get(url, {mdCode: this.mdCode}, (isSuccess, res) => {
          this.filters = res.Rows
          this.filters.forEach((f, index) => {
            let countUrl = `${this.BASEURL}/jdbc/common/basecommonlist/filterCount.do`
            this.post(countUrl, {mdCode: this.mdCode, searchParam: f.content}, (isSuccess, res) => {
