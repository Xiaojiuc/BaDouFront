...
<script>
import {Dialog, Tabs, TabPane, Form, FormItem, Select, Option} from 'element-ui'
...
export default {
    ...
    methods: {
        ...
        saveSingleBlock () {
            this.$refs['itemForm'].validate((valid) => {
                if (valid) {
                    ...
                    this.post(
                        this.INTERFACE.saveBlock,
                        { layoutId: this.item.layoutId, block: JSON.stringify(tempItem) },
                        (isSuccess, res) => {
        ...
        genReportField (value, isChange) {
            ...
            this.post(
                this.INTERFACE.packSettings,
                { dataSourceId: chartSourceId, dataSourceType: this.chartSourceType, reportShowTypeId: value },
                (isSuccess, res) => {
                    if (res.hasOk) {
